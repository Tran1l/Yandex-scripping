...
def get_data():
    ...
    for word in word_for_find:
        try:
            ...
            pass
        schet = 0
        dir_path_word = os.path.abspath(word)
        for j in range(0, 999):
            url = f'https://yandex.ru/images/search?text={word}&from=tabbar&p={j}&lr=75'
            response = requests.get(url=url, headers=headers, cookies=cookies)
            soup = BeautifulSoup(response.text, 'lxml')
            serp_list = soup.find('div', class_='serp-list')
            try:
                serp_items = serp_list.find_all('div', class_= 'serp-item')
            ...
            for i in serp_items:
                ...
                img_url = json_html['serp-item']['img_href']
                img_url_params = json_html['serp-item']['dups']
                schet += 1
                ...
                for key in result:
                    ...
                    if back > result[key]:
                        back = result[key]
                        url = key
                print(f'[INFO]Получено {schet} из {quantity_scriping} картинок в папку {word}')
                try:
                    response = requests.get(url=img_url)
                    img_option = open(f'{dir_path_word}/{schet}_big.jpg', 'wb')

                    img_option.write(response.content)
                    img_option.close()

                    small = 'https:' + json_html['serp-item']['thumb']['url']
                    response = requests.get(url=small)
                    img_option = open(f'{dir_path_word}/{schet}_small.jpg', 'wb')
                    img_option.write(response.content)
                    img_option.close()

                    try:
                        response = requests.get(url=url)
                        img_option = open(f'{dir_path_word}/{schet}_по_настройкам.jpg', 'wb')
                        img_option.write(response.content)
                        img_option.close()
                    except:
                        pass
